...
def perform_get(session: Session):
    ...
    try:
        data = storage.get_for_user(session.username, file, session.server_aes_key)
...
def perform_new(session: Session):
    ...
    storage.create_for_user(session.username, file, data, session.server_aes_key)
...
def perform_edit(session: Session):
    ...
    file = decode_utf8(session.sock.recv().body)
    try:
        contents = storage.get_for_user(session.username, file, session.server_aes_key)
        session.sock.send(
